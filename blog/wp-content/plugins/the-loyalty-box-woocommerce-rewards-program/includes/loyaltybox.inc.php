...
class Loyaltybox
{
    ...
    */
    ...
    {
        $option_name = 'wc_settings_tab_loyaltybox_error_logging';
        $check_switch = Loyaltybox::check_if_errors_enabled($option_name);
        $log = date('Y-m-d H:i:s.uP')." => ".$info.PHP_EOL;
        if ($first_line) {
            $log = PHP_EOL.PHP_EOL.$log;
        }
    ...
    {
        $option_name = 'wc_settings_tab_loyaltybox_request_logging';
        $check_switch = Loyaltybox::check_if_errors_enabled($option_name);
        $url = self::$endpoint;
    ...
    /**
     * On_off_for_error_reporting
     *
     * A method to check if the user has enabled error reporting
     *
     * @version 1.0.0
     *
     * @author Double Eye
     ...
     * @static
     */
    public static function check_if_errors_enabled($option_name)
    {
        global $wpdb;
        $values = get_transient( 'special_query_results' );
        if ( false === $values ) {
            // If it wasn't there, then regenerate the data and save the transient
            $special_query_results = $wpdb->get_results( "SELECT * FROM wp_options WHERE option_name = '$option_name'", OBJECT );
            set_transient( 'special_query_results', $special_query_results, HOUR_IN_SECONDS );
            $values = get_transient( 'special_query_results' );
        }
        if($values[0]->option_value == 'yes'){
            return true;
        } else {
            return false;
        }
