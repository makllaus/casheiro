			...
			<?php if(get_option('mvp_feat_post') == 'Featured Posts 1') { ?>
					...
					<div id="top-story-middle">
						<div id="middle-img">
							<?php $mvp_slider = get_option('mvp_slider'); if ($mvp_slider == "true") { ?>
								<?php if (isset($do_not_duplicate)) { global $do_not_duplicate; global $post; $recent = new WP_Query(array( 'post__not_in' => $do_not_duplicate, 'posts_per_page' => '1'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
								...
								<?php } endwhile; } ?>
							<?php } else { ?>
								<?php global $do_not_duplicate; global $post; $recent = new WP_Query('posts_per_page=1'); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
									<a href="<?php the_permalink(); ?>" rel="bookmark">
									<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
										<div class="top-middle-image">
											<?php the_post_thumbnail('post-thumb'); ?>
					...
					<div id="top-story-left">
						...
						<?php } else { ?>
							...
							<ul class="top-stories">
								<?php if (!empty($do_not_duplicate)) { global $do_not_duplicate; global $post; $current_category = get_option('mvp_featured_left'); $category_id = get_cat_ID($current_category); $recent = new WP_Query(array( 'cat' => $category_id, 'post__not_in' => $do_not_duplicate, 'posts_per_page' => '2'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
								<li>
									<a href="<?php the_permalink(); ?>" rel="bookmark">
									<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
				...
				<div id="top-story-right">
					...
					<?php } else { ?>
						...
						<ul class="top-stories">
							<?php if (!empty($do_not_duplicate)) { global $do_not_duplicate; global $post; $current_category = get_option('mvp_featured_right'); $category_id = get_cat_ID($current_category); $recent = new WP_Query(array( 'cat' => $category_id, 'post__not_in' => $do_not_duplicate, 'posts_per_page' => '2'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (!empty($do_not_duplicate)) { ?>
							<li>
			...
			<?php } else if(get_option('mvp_feat_post') == 'Featured Posts 2') { ?>
				<?php $mvp_slider = get_option('mvp_slider'); if ($mvp_slider == "true") { ?>
					...
					<div id="feat1-left-wrapper">
						<?php if (!empty($do_not_duplicate)) { global $do_not_duplicate; global $post; $recent = new WP_Query(array( 'post__not_in' => $do_not_duplicate, 'posts_per_page' => '1'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
							<a href="<?php the_permalink(); ?>" rel="bookmark">
							<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
				...
				<?php } else { ?>
					<div id="feat1-main-wrapper">
						<?php global $do_not_duplicate; global $post; $recent = new WP_Query(array( 'posts_per_page' => '1'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
							<a href="<?php the_permalink(); ?>" rel="bookmark">
							<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
								<div class="feat1-main-img">
									<?php the_post_thumbnail('post-thumb'); ?>
				...
				<div class="home-widget">
					<ul class="home-list infinite-content">
						<?php $mvp_slider = get_option('mvp_slider'); $mvp_posts = get_option('mvp_featured_posts'); if (($mvp_slider == "true") || ($mvp_posts == "true")) { ?>
							<?php if (isset($do_not_duplicate)) { global $do_not_duplicate; global $post; $mvp_posts_num = get_option('mvp_posts_num'); $paged = (get_query_var('page')) ? get_query_var('page') : 1; query_posts(array( 'posts_per_page' => $mvp_posts_num, 'post__not_in'=>$do_not_duplicate, 'paged' =>$paged )); if (have_posts()) : while (have_posts()) : the_post(); ?>
							<li class="infinite-post">
								<a href="<?php the_permalink(); ?>" rel="bookmark">
							...
							<?php endwhile; endif; } ?>
						<?php } else { ?>
							<?php global $do_not_duplicate; global $post; $mvp_posts_num = get_option('mvp_posts_num'); $paged = (get_query_var('page')) ? get_query_var('page') : 1; query_posts(array( 'posts_per_page' => $mvp_posts_num, 'paged' =>$paged )); if (have_posts()) : while (have_posts()) : the_post(); ?>
							<li class="infinite-post">
								<a href="<?php the_permalink(); ?>" rel="bookmark">
								<div class="home-list-img">
									<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
